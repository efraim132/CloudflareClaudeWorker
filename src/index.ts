...
function buildIncidentEmbed(data: StatuspagePayload): DiscordEmbed {
    ...
    if (!latestMessage && incident.incident_updates && incident.incident_updates.length > 0) {
    ...
    let componentText = "";
    if (incident.current_affected_components && incident.current_affected_components.length > 0) {
...
async function sendToDiscord(webhookUrl: string, embed: DiscordEmbed): Promise<Response> {
    const payload = {
...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        if (url.pathname === "/webhook" && request.method === "POST") {
            ...
            // Forward to Discord
            const discordResponse = await sendToDiscord(env.DISCORD_WEBHOOK_URL, embed);
            const discordBody = await discordResponse.text();

            // Log it (shows up in `wrangler tail` for debugging)
            console.log(`Discord responded: ${discordResponse.status} - ${discordBody}`);
